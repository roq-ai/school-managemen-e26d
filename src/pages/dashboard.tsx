...
import { getTimetables } from 'apiSdk/timetables';
import { TimetableInterface } from 'interfaces/timetable';
import { getFees } from 'apiSdk/fees';
...
function DashboardPage() {
  ...
  const timetableStat = useSWR<PaginatedInterface<TimetableInterface>>(
    () => `/timetables`,
    () => getTimetables({ limit: 1, offset: 0 }),
  );

  const feeStat = useSWR<PaginatedInterface<FeeInterface>>(
    () => `/fees`,
    () => getFees({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Timetables',
      result: timetableStat?.data?.totalCount,
      isLoading: timetableStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Fees',
      result: feeStat?.data?.totalCount,
      isLoading: feeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
